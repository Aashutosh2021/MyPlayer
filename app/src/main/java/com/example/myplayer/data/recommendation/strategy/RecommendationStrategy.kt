...
interface RecommendationStrategy {
    fun scoreAndRank(seedSong: OnlineSong, candidates: List<RecommendationSong>): List<RecommendationSong>
}

@Singleton
class DefaultRankingStrategy @Inject constructor() : RecommendationStrategy {
    override fun scoreAndRank(seedSong: OnlineSong, candidates: List<RecommendationSong>): List<RecommendationSong> {
        return candidates.map { candidate ->
            var score = 0
            val reasons = mutableListOf<String>()

            // Score Same Artist
            if (candidate.artist.equals(seedSong.artist, ignoreCase = true)) {
                score += RecommendationConstants.SCORE_SAME_ARTIST
                reasons.add("Same Artist")
            }

            // Simple Title/Album matching (fuzzy)
            // Note: Since OnlineSong doesn't have an album field, we only have artist and title.
            val seedTokens = seedSong.title.lowercase().split(Regex("\\W+"))
